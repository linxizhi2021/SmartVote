...
class VoteController extends BaseProjectController {
	...
	async getVoteList() {
		...
		for (let k = 0; k < list.length; k++) { 
			list[k].start = timeUtil.timestamp2Time(list[k].VOTE_START, 'Y-M-D h:m');
			list[k].end = timeUtil.timestamp2Time(list[k].VOTE_END, 'Y-M-D h:m');
			list[k].weekStart = timeUtil.week(timeUtil.timestamp2Time(list[k].VOTE_START, 'Y-M-D'));
			list[k].weekEnd = timeUtil.week(timeUtil.timestamp2Time(list[k].VOTE_END, 'Y-M-D'));
	...
	async viewVote() {
		...
		let service = new VoteService();
		let vote = await service.viewVote(this._userId, input.id, input.isPv);
	...
	async doVote() {
		...
		let service = new VoteService();
		return await service.doVote(this._userId, input.voteId, input.idx);
	}
