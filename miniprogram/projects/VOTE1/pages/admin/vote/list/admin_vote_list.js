...
Page({
			...
			success: async res => {
				switch (res.tapIndex) {
					case 0: { //预览
						...
						let theme = pageHelper.dataset(e, 'theme');
						wx.navigateTo({
							url: '../../../vote/detail/vote_detail?id=' + id + '&theme=' + theme,
						});
						break;
					}
					case 1: { //置顶 
	...
	_statAll: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/vote_stat_all', params, opts).then(res => {
				pageHelper.showModal('统计完成，请刷新列表查看最新票数');
			});
		} catch (err) {
			console.log(err);
		}
	...
	_getSearchMenu: function () {
		...
		let sortItem2 = [
			...
			{ label: '按人数', type: 'sort', value: 'user' },
			{ label: '按票数', type: 'sort', value: 'cnt' },
			{ label: '按开始时间', type: 'sort', value: 'start' },
			{ label: '按截止时间', type: 'sort', value: 'stop' },
		];
		...
		let sortMenus = [
			...
			{ label: '首页推荐', type: 'sort', value: 'vouch' },
